...
import EditBlogger from "./EditBlogger";
    ...
    cell: ({ row }) => {
      return (
        <div className="flex items-center gap-3">
          <Link href={`bloggers/${row.original.id}`}>
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${baseURL}/api/User/get/all-bloggers-by-popularity/${month}`,
          {
            headers: {
  ...
  return (
    <div>
      <div className="float-end">
        <Select onValueChange={(value) => setMonth(parseInt(value as string))}>
        ...
        <div>
          <div className="w-full">
            <div className="flex items-center py-4">
              <Input
                placeholder="Filter bloggers name..."
                value={
